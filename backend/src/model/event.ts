...
export function addEvent(name : string, maxPrice : number, startDate : string, endDate : string, id_owner: number) {
    return new Promise((result: (value:any) => void, rej) => {
...
export function suppEvent(id : number) {
    return new Promise((result, rej) => {
        con.query("DELETE FROM events WHERE id=?", [id], (err, res) => {
            if (err)
                rej(err);
            else
                result(res);
...
export function modifEvent(name : string, maxPrice : number, id : number, id_owner : number) {
    return new Promise((result, rej) => {
        con.query("UPDATE events set name = ? , maxPrice = ? WHERE id=? and id_owner = ?", [name, maxPrice, id, id_owner], (err, res) => {
            if (err)
                rej(err);
            else
                result(res);
...
export function getEventById(id : number) {
    return new Promise((result, rej) => {
        con.query("select count(id) as nb from events where id=?", [id], (err, res) => {
            if (err)
                rej(err);
            else
                result(res);
